...
Views.diag = async (q)=>{
  ...
  c.appendChild(el('div','card',
    `<div class="head-row"><span class="name">${esc(d.name)}</span><span class="code">${d.code}</span>
     ...
     <div class="note">📅 ${d.asOf} 收盤・昨收 ${fmt(d.prevClose)}｜來源 ${esc(d.prov.source)}・${d.prov.demo?'示範資料':'正式'}｜抓取 ${esc(d.prov.fetchedAt)}</div>`));
  // 總覽
  ...
  box.appendChild(mk('預估本益比',S.estPE.value!=null?fmt(S.estPE.value,1):'<span class="nodata">無估值</span>',S.estPE.value!=null?`落點 ${esc(S.estPE.layer||'')}`:esc(S.estPE.reason||''),`<div class="explain">🔎 本平台自有估值模型；${S.estPE.value==null?'獲利太薄或資料不足時不硬估。':'見估值河流圖。'}</div>`+(S.estPE.value!=null?`<div class="row"><span></span><span class="lk" onclick="Router.go('valuation',{code:'${d.code}'})">看估值河流圖 →</span></div>`:'')));
  ...
  box.appendChild(mk('法人買賣超',S.inst?`${S.inst.net5>0?'+':''}${fmtInt(S.inst.net5)} 張`:'<span class="nodata">無資料</span>',S.inst?`近5日｜今日 ${S.inst.today>0?'+':''}${fmtInt(S.inst.today)}`:'',S.inst?`<div class="row"><span>外資/投信/自營</span><b>${fmtInt(S.inst.foreign)}/${fmtInt(S.inst.invest)}/${fmtInt(S.inst.dealer)}</b></div><div class="row"><span>近10日</span>${spark(S.inst.spark10)}</div>`:''));
